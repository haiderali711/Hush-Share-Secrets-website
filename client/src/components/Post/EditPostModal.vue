  ...
  <modal :show="show" @close="close($event)">
    <form @submit.stop.prevent="handleSubmit($event)">
      ...
      <div class="modal-footer text-right">
        ...
        <button class="modal-default-button" :disabled="!formIsValid" @click="handleSubmit($event)">Update</button>
      </div>
    </form>
  </modal>
</template>
...
export default {
  ...
  watch: {
    show() {
      if (this.show) this.postContent = this.post.content;
    }
  },
  methods: {
    handleSubmit(e) {
      ...
      Api.patch(`posts/${this.post._id}`, { content: this.postContent })
        .then(response => {
